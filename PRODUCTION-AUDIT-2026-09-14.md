...
**Resolution:** Laravel’s native `AuthenticateSession` is registered in the `web` middleware group. Public pages, protected application routes, and package routes using `web` reject revoked sessions on their next request. Guests can still visit public pages. The custom auth-alias wrapper and public-route exclusions have been removed. The password-update method now matches upstream: it saves the password and returns the existing success response. The middleware refreshes the current session’s fingerprint after the update and rejects other sessions on their next request. Old remember-me cookies, including the current browser’s, cannot restore authentication after the password changes. The current session stays signed in, but its ID and CSRF token are not explicitly rotated. Custom remember-token rotation, re-login, and confirmation-timestamp clearing were removed from the controller. Password resets retain Fortify’s existing behavior. Regression tests cover independent browser sessions, idle sessions, remembered authentication, same-password changes/resets, failed attempts, and database/cookie session drivers.

**Framework dependency (September 16):** The custom login-fingerprint middleware was removed in favor of Laravel’s native `AuthenticateSession`. Login-time fingerprint capture requires a framework version containing [laravel/framework#61594](https://github.com/laravel/framework/pull/61594). The currently installed v13.32.0 does not contain the fix; upgrade to a version containing it before relying on protection for sessions that have not followed the login redirect. The regression tests remain enabled. The web-group configuration passed all 184 PHP tests (1,901 assertions) against the exact PR head in an isolated copy. Pint and PHPStan passed. Four targeted scenarios also pass on the installed framework: established sessions are revoked on their next public, application, Fortify, or newly registered package web request, and public pages remain accessible afterward as a guest. The source checkout’s dependencies are unchanged.
...
- [ ] **R04 — Passkey UX and real-device gate.** Enrollment loading feedback was addressed by F17; cancellation guidance and error separation were updated on September 17 (details below). The remaining gate is real-device verification: enrollment, login, confirmation, cancellation, timeout, deletion, renamed/long credential labels, and recovery on platform authenticators/security keys over HTTPS. Do not label passkeys fully certified from software-authenticator or route/ownership tests alone.
...
### R04 follow-up — September 17, 2026

**Implemented:** Enrollment retains the instruction to complete the device prompt and the existing delayed spinner. Cancellation/timeouts show neutral status feedback ("Passkey setup was cancelled or timed out. Please try again.") and preserve the entered name. Other passkey-operation failures appear below the action buttons; only server-side name-validation errors mark the name field invalid. Feedback clears when starting another authenticator attempt or closing the form. The form's Cancel button still closes the form only when idle; it does not promise to cancel network requests or a credential already being saved. The extra cancellation instruction was removed at the maintainer's request.

**Verified:** Frontend formatting/lint, TypeScript, production build, Pint, PHPStan, and all 186 PHP tests (1,918 assertions) pass. shadcn still reports Base UI / `base-nova` / `b37ZhrNTs`; the Radix scan is clean. In a disposable app, simulated authenticator cancellation, timeout, and duplicate-registration errors restore usable controls, preserve the name, and do not set `aria-invalid` on it. Retry returns to the pending state. An actual HTTP 503 options response shows an operation error and restores the controls. Empty-name validation still focuses and marks the name field; closing and reopening clears operation feedback. Screenshots show the rendered pending and cancellation states with a simulated authenticator. Existing PHP software-authenticator tests cover successful registration, confirmation, and deletion. No real platform authenticator, hardware key, screen reader, or successful browser authenticator ceremony was tested in this follow-up. R04 stays open for the real-device gate.
