...
class PasswordConfirmationPolicyTest extends TestCase
{
        ...
        foreach (['profile.edit', 'security.edit'] as $route) {
            $this->actingAs($user)->get(route($route))->assertOk()
                ->assertInertia(fn (Assert $page) => $page
                    ->missing('auth.user.password')->missing('auth.user.two_factor_secret')
    ...
    {
        foreach (Route::has('password.confirm.store') ? [true, false] : [false] as $enabled) {
            $this->flushSession();
    ...
    public function test_confirmation_returns_server_validation_messages_before_checking_the_password(): void
    {
        if (! Route::has('password.confirm.store')) {
            $this->markTestSkipped('Confirmation removed.');
        }
