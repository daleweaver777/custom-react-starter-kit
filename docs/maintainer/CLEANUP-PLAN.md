...
The app requires PHP `^8.5` in `composer.json` and Node `^24.21.0` in `package.json` (the current Node 24 LTS release). Source CI and the installed application workflow read `require.php` and `engines.node` directly from those manifests; PHP setup updates to the latest required-series patch and Node setup selects the latest matching release. The extra PHP/Node version files and their packaging requirements were removed at the maintainer's request. Node types match the Node 24 series. These repository settings do not configure Herd or deployment runtimes.

Evidence is in `/private/tmp/starter-ci-35265399522`: original failure artifacts under `ci-{0,1,3}`, diagnostic raw mail under `run26-3`, and `php83-jit-comparison.json` / `php8510-jit-comparison.json`. Earlier source checks passed (34 application tests and 187 maintainer tests), as did Base UI/base-nova/b37ZhrNTs verification, four fresh Node archive installations, and all 12 account-lifecycle cases on those installations. Initial local TypeScript errors were missing generated Wayfinder imports; generation restored a passing check. Final local source checks pass on Herd PHP 8.5.8 and Node 24.21.0: 34 application tests / 113 assertions, 187 maintainer tests / 2,815 assertions, formatting, lint, PHPStan, TypeScript, client/SSR builds, and React Doctor. The focused packaging test also passes all four selections (876 assertions). [Run 35285259122](https://github.com/daleweaver777/custom-react-starter-kit/actions/runs/35285259122) passed source checks, all four production browser suites, and all Node/no-Node installer checks on PHP 8.5.10 / Node 24.21.0; its remaining failure was the development SSR fixture's Vite startup guard under `CI=1`.

The browser configuration now sets Laravel Vite's supported `LARAVEL_BYPASS_ENV_CHECK=1` only on the disposable development SSR server, bound to loopback. A local `CI=1` run reproduced the original startup failure; after the scoped override, development SSR and production CSR each pass, including rendered HTML, hydration, profile edits/reset, and pending saves (`dev-ci-before.log`, `dev-ci-after.log`, `csr-ci-after.log`). These focused controls use the existing Herd PHP 8.5.8 / Node 22.23.2; final CI validates the declared Node 24 LTS runtime. Final acceptance: [run 35286030841](https://github.com/daleweaver777/custom-react-starter-kit/actions/runs/35286030841) is green for commit `2da701f`: the source job and all four installer jobs pass, including every Node/archive and no-Node selection, the three-browser production suites, development SSR, and production CSR on PHP 8.5.10 / Node 24.21.0. Temporary runtime installations used during diagnosis were removed; Herd defaults and configuration were preserved.

### CI action runtime update (2026-09-17)

The source workflow now pins `actions/upload-artifact` v7.0.1 at `043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`. Its official action definition declares Node 24, resolving the warning from v4.6.2's Node 20 action runtime. Artifact names, paths, archive behavior, and retention remain unchanged. The application workflow fixture uses the same manifest readers as source CI, with no hardcoded runtime versions or auxiliary version files. PHP extraction strips the leading caret from the current `^8.5` requirement; setup-node natively reads `package.json`. Focused packaging tests pass for all four selections (868 assertions). A fresh full CI run verifies the workflow and artifact uploads.
...
- [x] Run the configured remote CI: PHP 8.5.10 / Node 24.21.0 LTS passed all five jobs in [run 35286030841](https://github.com/daleweaver777/custom-react-starter-kit/actions/runs/35286030841).
